      ...
      {NAV.map(([icon, label, key]) => (
        <button
          ...
          style={{
            ...
            gap: 1,
            padding: "6px 8px",
            background: page === key ? C.accent + "18" : "transparent",
            border:
              page === key
                ? `1px solid ${C.accent}33`
                : "1px solid transparent",
            borderRadius: 8,
            color: page === key ? C.accent : C.muted,
            fontSize: 9,
            cursor: "pointer",
            minWidth: 48,
          }}
...
export function RiskGauge({ value }: { value: number }) {
  ...
  return (
    <svg width={110} height={70} viewBox="0 0 110 70">
      ...
      <text x={55} y={64} textAnchor="middle" fill={C.muted} fontSize={9}>
        {RISK_L[lv]}
      </text>
    </svg>
  );
  ...
  return (
    <Card>
      ...
      >
        ...
        <span
          style={{ fontSize: 10, color: C.accent, fontFamily: "monospace" }}
        >
          {current} / {total}
        </span>
      </div>
