...
export function FramePlayer({
  ...
  useEffect(() => {
    ...
    // 비디오 원본 크기 → 표시 크기 비율
    // 서버가 640x480으로 리사이즈해서 처리하므로, 박스 좌표는 640x480 기준
    const PROC_W = 640,
      PROC_H = 480;
    const scaleX = rect.width / PROC_W;
    const scaleY = rect.height / PROC_H;
    ...
    boxes.forEach((box: any) => {
      ...
      // 박스
      ctx.strokeStyle =
        risk > 70 ? "#EF4444" : risk > 40 ? "#F59E0B" : "#22C55E";
      ctx.lineWidth = 2;
      ...
      >
        <video
          ...
          onLoadedMetadata={() => {
            ...
            setVideoSize({
              w: videoRef.current?.clientWidth || 640,
              h: videoRef.current?.clientHeight || 480,
            });
          }}
      ...
      >
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: 10, color: "#94A3B8" }}>
            프레임 #{resultIdx >= 0 ? resultIdx : 0} | 위험도 {risk.toFixed(0)}
          </div>
        </div>
      </div>
    </div>
