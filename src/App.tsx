      ...
      >
          ...
          <>
              ...
              {mode === "camera" && (
                <button
                  ...
                  style={{
                    padding: "7px 14px",
                    borderRadius: 8,
                    border: `1px solid ${roiDrawMode ? C.accent : C.border}`,
                    background: roiDrawMode ? C.accent + "18" : "transparent",
                    color: roiDrawMode ? C.accent : C.muted,
                    fontSize: 11,
                    fontWeight: 600,
                    cursor: "pointer",
                  }}
            ...
            {!hasData && mode === "idle" ? (
              ...
              >
                ...
                >
                  영상 파일 업로드 시 서버에서 프레임을 분석하여 결과를
                  실시간으로 보여줍니다. 카메라 모드에서는 브라우저 카메라로
                  실시간 감시가 가능합니다.
                </div>
              </div>
            ) : (
              <>
                ...
                >
                  ...
                  >
                    ...
                    >
                      T=
                      {lastResult?.motion?.threshold_used ||
        ...
        {page === "roi" && (
          <>
            ...
            >
              클릭으로 꼭짓점을 추가하고, 더블클릭으로 다각형을 완성하세요.
              우클릭으로 삭제할 수 있어요.
              {mode === "camera" &&
                " 대시보드에서 🎯 ROI 그리기 버튼을 누르면 카메라 영상 위에서도 그릴 수 있어요."}
            </div>
        ...
        {page === "alerts" && (
          <>
            ...
            {alerts.length === 0 ? (
              ...
              >
                영상을 분석하면 위험도 {settings.alert_threshold} 이상일 때
                알림이 기록됩니다
              </div>
            ) : (
              alerts.map((a, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div
                        style={{ fontSize: 10, color: "#CBD5E1", marginTop: 4 }}
                      >
                        {a.message}
                      </div>
                      <div style={{ fontSize: 9, color: C.dim, marginTop: 2 }}>
                        ...
                        {a.motion_pixels}px
                        {a.boxes?.some((b) => b.in_roi) &&
                          ` | ROI: ${a.boxes.find((b) => b.in_roi)?.in_roi}`}
                      </div>
                    </div>
                  </div>
                );
            ...
            {done ? (
              <Card style={{ marginBottom: 10 }}>
                ...
                >
                  <StatCard
                    label="분석 프레임"
                    value={done.analyzed_frames || 0}
                    unit="장"
                    color={C.accent}
                  />
                  <StatCard
                    label="감지 횟수"
                    value={done.total_detections || 0}
                    unit="회"
                    color="#EF4444"
                  />
                  <StatCard
                    label="평균 위험도"
                    value={done.summary?.avg_risk?.toFixed(1) || 0}
                    unit=""
                    color="#F59E0B"
                  />
                  <StatCard
                    label="최대 위험도"
                    value={done.summary?.max_risk?.toFixed(1) || 0}
                    unit=""
                    color="#EF4444"
                  />
                </div>
            ...
            {(
              [
                ...
                },
                {
                  ...
                  desc: "N프레임 평균으로 랜덤 노이즈 제거",
                },
                {
                  key: "use_shadow_removal",
                  label: "Shadow Removal",
                  desc: "HSV 기반 그림자 오탐 제거",
                },
                {
                  key: "use_temporal_smoothing",
                  label: "Temporal Smoothing",
                  desc: "연속 프레임 확인으로 단발성 오탐 제거",
                },
                {
                  key: "use_adaptive_threshold",
                  label: "Adaptive Threshold",
                  desc: "영역별 자동 임계값",
                },
                {
                  key: "use_dynamic_threshold",
                  label: "Dynamic Threshold",
                  desc: "밝기에 따라 임계값 자동 조정",
                },
                {
                  key: "skip_unchanged_frames",
                  label: "변화 없는 프레임 스킵",
                  desc: "대역폭 절약",
                },
              ] as const
              ...
              <Card key={s.key} style={{ marginBottom: 4 }}>
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      updateSetting(s.key, !(settings as any)[s.key])
                    }
                    style={{
                      width: 38,
                      height: 22,
                      ...
                      border: "none",
                      cursor: "pointer",
                      background: (settings as any)[s.key]
                        ? C.accent
                        : C.border,
                      position: "relative",
                  ...
                  >
                    <div
                      style={{
                        ...
                        position: "absolute",
                        top: 3,
                        left: (settings as any)[s.key] ? 19 : 3,
            ...
            <Card>
              <div style={{ display: "flex", gap: 6 }}>
                {(["binary", "base64"] as const).map((m) => (
                  <button
                    key={m}
                    onClick={() => updateSetting("transfer_mode", m)}
                    style={{
                      padding: "5px 14px",
                      borderRadius: 6,
                      border: `1px solid ${settings.transfer_mode === m ? C.accent : C.border}`,
                      background:
                        settings.transfer_mode === m
                          ? C.accent + "15"
                          : "transparent",
                      color: settings.transfer_mode === m ? C.accent : C.muted,
                      fontSize: 11,
                      cursor: "pointer",
                    }}
                  >
