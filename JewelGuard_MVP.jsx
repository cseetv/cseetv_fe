...
function histEq(gray) {
  ...
  const mn = cdf.find((v) => v > 0),
    lut = cdf.map((v) => Math.round(((v - mn) / (n - mn)) * 255));
...
function useVideoFrames() {
    ...
    return new Promise((resolve) => {
      ...
      video.onloadedmetadata = () => {
        const info = {
          ...
          fps: 30, // 기본값, 정확한 fps는 브라우저에서 구하기 어려움
          name: file.name,
  ...
  const extractFrame = useCallback((time) => {
    return new Promise((resolve) => {
...
// UI Components
// ============================================================
const RISK_C = { safe: "#10B981", warn: "#F59E0B", danger: "#EF4444" };
const RISK_L = { safe: "정상", warn: "주의", danger: "위험" };
const DIAG_L = {
  under_exposed: "저조도",
  over_exposed: "과노출",
  low_contrast: "저대비",
...
function Histogram({ hist, color = "#6366F1", height = 52, label }) {
  if (!hist) return null;
...
function MiniChart({ data, color = "#6366F1", h = 32 }) {
...
export default function App() {
  ...
  const [inputType, setInputType] = useState(null); // "image" | "video"
  ...
  const handleVideoUpload = useCallback(
    async (file) => {
      ...
      if (frame) {
        ...
        analyzeGray(frame.gray, frame.w, frame.h, frame.url);
        setFrameIdx(0);
      }
    },
  ...
  // ---- Video playback (frame-by-frame analysis) ----
  const stepInterval = 0.2; // 0.2초 간격으로 프레임 추출 (5fps 분석)
  ...
  useEffect(() => {
    ...
    }, 300); // 300ms마다 한 프레임 (분석 포함)

    return () => clearInterval(playRef.current);
  }, [playing, videoInfo, extractFrame, analyzeGray]);

  // ---- File input handler (image or video) ----
          ...
          <>
            ...
            >
              ...
              >
                ...
                <input
                  type="file"
                  accept="image/*,video/mp4,video/webm,video/ogg,.mp4,.webm,.ogg,.mov,.avi"
                  onChange={(e) => handleFile(e.target.files?.[0])}
                  style={{ display: "none" }}
                />
              </label>
              <span style={{ fontSize: 10, color: "#334155" }}>
                이미지: JPG, PNG | 영상: MP4, WebM, OGG, MOV
              </span>
            </div>
            ...
            {!analysis ? (
              ...
              >
                ...
                >
                  ...
                  때마다 이전 프레임과 비교하여 움직임 감지
                  <br />
                  <strong style={{ color: "#10B981" }}>영상 (MP4)</strong>:
                  업로드 후 ▶ 버튼으로 프레임별 자동 분석
                </div>
              </div>
            ) : (
              <>
                  ...
                  >
                    ...
                    >
                      <span style={{ fontSize: 9, color: "#475569" }}>
                        {videoInfo.width}×{videoInfo.height} | {videoInfo.size}{" "}
                        | {videoInfo.type}
                      </span>
                      <span style={{ fontSize: 9, color: "#475569" }}>
                        분석: 5fps | 알림: {alerts.length}건
                      </span>
                    </div>
                  </div>
                )}
                    ...
                    >
                      ...
                      >
                        <Badge
                          ...
                          text={DIAG_L[analysis?.diagnosis]}
                        />
                ...
                >
                  ...
                  <StatCard
                    ...
                    unit="px"
                    color={motion?.motionPixels > 1000 ? "#EF4444" : "#64748B"}
                  />
                </div>
          ...
          <>
            ...
            >
              ...
              <div style={{ fontSize: 9, color: "#94A3B8", marginTop: 6 }}>
                N↑ → SNR↑, 이론값과 근접. N=100에서 차이 발생 → 양자화 에러 영향
              </div>
            </div>
            {/* Filters */}
            <div
            ...
            >
              ...
              <div style={{ fontSize: 9, color: "#94A3B8", marginTop: 6 }}>
                Gaussian 5×5가 PSNR+에지 최적. Ideal은 링잉 발생.
              </div>
            </div>
          </>
        )}
          ...
          <>
            ...
            >
              ...
              <div style={{ fontSize: 10, color: "#94A3B8", lineHeight: 1.7 }}>
                <strong style={{ color: "#6366F1" }}>이미지</strong>: JPG, PNG,
                GIF, WebP (모든 브라우저)
                <br />
                <strong style={{ color: "#10B981" }}>영상</strong>: MP4 (H.264),
                WebM (VP8/VP9), OGG (Theora)
                <br />
                <span style={{ color: "#64748B" }}>
                  ※ MP4가 가장 안전 (모든 브라우저 지원). AVI는 브라우저에서
                  지원 안 됨.
                </span>
              </div>
            </div>
